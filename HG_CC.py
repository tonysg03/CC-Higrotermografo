...
while not command == "S":
    if command == "C":
        ...
        if command_option == "S":
            ...
            # proc = "se_sumó_"+ str(valor)
            # T_proc.extend([str(proc)])
        elif command_option == "R":
            file.loc[(file.fecha.ge(df_fecha_rango["fecha"][0])) & (file.fecha.le(df_fecha_rango["fecha"][1])), "Datos_NaN"] -= valor
            T_mod_val = file.loc[(file.fecha.ge(df_fecha_rango["fecha"][0])) & (file.fecha.le(df_fecha_rango["fecha"][1]))]
            T_mod_val["procedimiento"] = "se_resta_"+str(valor)
            T_mod.extend(T_mod_val.values)
            # proc = "se_restó_"+ str(valor)
            # T_proc.extend([str(proc)])
        else:
...
for x in range(0, round(len(file["fecha"])/149)):
    ...
    file["procedimiento"] = "se_suma_"+str((100 - valor_max))
...
out_mod_file_T = out_file[["cuenca","estacion","fechas","horas","T"]].copy()
...
out_mod_file_T.to_csv('modif_T.csv', sep=",", na_rep="NaN", index=False)
...
out_mod_file_H .to_csv('modif_H.csv', sep=",", na_rep="NaN", index=False)